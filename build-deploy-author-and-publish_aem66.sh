...
# Build application
./build-deploy.sh build --maven.profiles=fast,aem66 "$@"
if [ "$?" -ne "0" ]; then
  pause_message
  exit $?
fi
...
./build-deploy.sh deploy --maven.profiles=fast,aem66 "$@" &

# Deploy to publish (in parallel)
./build-deploy.sh deploy --maven.profiles=fast,publish,aem66 "$@" &
